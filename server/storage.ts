...
export class MemStorage implements IStorage {
  ...
  // Content Issues methods
  async getContentIssues(): Promise<any[]> {
    try {
      const { data, error } = await supabase
        .from('content_issues')
        .select('*')
      ...
      console.error('Storage error getting content issues:', error);
      throw error;
    }
  }

  async createContentIssue(issueData: {
    contentTitle: string;
    contentType: string;
    contentId: string;
    ...
    try {
      const { data, error } = await supabase.rpc('insert_content_issue', {
        p_content_title: issueData.contentTitle,
        p_content_type: issueData.contentType,
        p_content_id: issueData.contentId,
        p_season_number: issueData.seasonNumber,
        p_episode_number: issueData.episodeNumber,
        p_description: issueData.description,
        p_user_ip: issueData.userIP
      });

      if (error) throw error;
      return data;
    } catch (error) {
      console.error('Storage error creating content issue:', error);
      throw error;
    }
  }

  async deleteContentIssue(issueId: string): Promise<void> {
    try {
      ...
      console.error('Storage error deleting content issue:', error);
      throw error;
    }
  }

  async updateContentIssueStatus(issueId: string, status: string): Promise<void> {
    try {
      const { error } = await supabase
        .from('content_issues')
      ...
      console.error('Storage error updating content issue status:', error);
      throw error;
    }
  }
}
...
// Add content issues methods to the main storage object
storage.getContentIssues = storage.getContentIssues;
storage.createContentIssue = storage.createContentIssue;
storage.deleteContentIssue = storage.deleteContentIssue;
storage.updateContentIssueStatus = storage.updateContentIssueStatus;
