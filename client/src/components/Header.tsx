    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-background/80 backdrop-blur-md border-b border-border/50">
      <div className="container mx-auto px-3 sm:px-4 md:px-6 py-3 md:py-4">
        <div className="flex items-center justify-between relative">
          <Link to="/" className="flex items-center space-x-1 sm:space-x-2 text-2xl font-bold text-primary z-10 relative">
            <img 
              src="/logo.png" 
              alt="GreenFlix Logo" 
          ...
          <nav className="absolute left-1/2 transform -translate-x-1/2 flex space-x-6 lg:space-x-8 z-0 hidden lg:flex">
            {navItems.map((item) => {
          ...
          <div className="relative z-10 flex-shrink-0">
            ...
            <button 
              className="lg:hidden text-muted-foreground hover:text-primary p-1 transition-colors ml-2"
              onClick={toggleMobileMenu}
            >
              {isMobileMenuOpen ? <X className="w-5 h-5 stroke-1" /> : (
          ...
          {/* Mobile Navigation */}
          {isMobileMenuOpen && (
            <div className="absolute top-full left-0 right-0 mt-2 bg-background/90 backdrop-blur-md border border-border/50 rounded-lg lg:hidden z-50">
                ...
                {navItems.map((item) => {
                  ...
                  return (
                    <Link
                      key={item.path}
                      to={item.path}
                      className={`flex items-center space-x-2 text-sm font-medium transition-all duration-200 hover:text-primary whitespace-nowrap ${
                        location.pathname === item.path
                          ? 'text-primary border-b-2 border-primary'
                          : 'text-muted-foreground hover:text-foreground'
                      }`}
